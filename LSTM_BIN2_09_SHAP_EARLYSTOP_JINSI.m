...
fprintf('\n===== 最佳模型评估结果（训练集AUC≤0.95）=====\n');
fprintf('训练集：准确率=%.4f, 精确率=%.4f, 召回率=%.4f, F1=%.4f, AUC=%.4f, LogLoss=%.4f\n', ...
        train_acc, train_precision, train_recall, train_f1, train_auc_final, train_logloss);
fprintf('测试集：准确率=%.4f, 精确率=%.4f, 召回率=%.4f, F1=%.4f, AUC=%.4f, LogLoss=%.4f\n', ...
        test_acc, test_precision, test_recall, test_f1, test_auc, test_logloss);
fprintf('最佳阈值: %.4f\n', best_threshold);

% 检查过拟合情况
overfit_auc = train_auc_final - test_auc;
overfit_f1 = train_f1 - test_f1;
fprintf('\n过拟合检查：\n');
fprintf('AUC过拟合差值: %.4f (应<0.1)\n', overfit_auc);
fprintf('F1过拟合差值: %.4f (应<0.1)\n', overfit_f1);

% 可视化结果
% ROC曲线
figure('Name', 'ROC曲线', 'Position', [200, 200, 800, 500]);
[fpr_test, tpr_test] = roc_curve(T_test_aligned, ensemble_test_prob);
plot(fpr_test, tpr_test, 'b-', 'LineWidth', 1.5, 'DisplayName', ['测试集 (AUC = ' num2str(test_auc, '%.4f') ')']);
hold on;
plot([0, 1], [0, 1], 'k:', 'LineWidth', 1);
xlabel('假正例率 (FPR)', 'FontSize', 12);
ylabel('真正例率 (TPR)', 'FontSize', 12);
title('ROC曲线（训练集AUC≤0.95）', 'FontSize', 14, 'FontWeight', 'bold');
grid on;
...
% PR曲线
figure('Name', 'Precision-Recall曲线', 'Position', [200, 200, 800, 500]);
[precision_vals, recall_vals, thresholds_pr] = precision_recall_curve(T_test_aligned, ensemble_test_prob);
plot(recall_vals, precision_vals, 'b-', 'LineWidth', 1.5);
xlabel('召回率 (Recall)', 'FontSize', 12);
ylabel('精确率 (Precision)', 'FontSize', 12);
title('Precision-Recall曲线', 'FontSize', 14, 'FontWeight', 'bold');
grid on;
set(gca, 'FontSize', 10);
set(gcf, 'color', 'w');
...
% 选择测试集中的样本进行SHAP分析
n_samples = min(1000, size(p_test, 1));  % 最多分析100个样本
sample_indices = randperm(size(p_test, 1), n_samples);

% 创建背景数据集（用于计算基准值）
n_background = min(100, size(p_train, 1));  % 减少背景样本数以提高计算效率
background_indices = randperm(size(p_train, 1), n_background);
X_background = p_train(background_indices, :);

% 选择要解释的样本
X_explain = p_test(sample_indices, :);

% 使用近似方法计算SHAP值（替换原来的核函数方法）
...
for i = 1:n_samples
    ...
    % 计算SHAP值（使用近似实现）
    shap_values(i, :) = compute_shap_values_approx(best_booster, x_instance, X_background, best_iteration);
end
...
% 将工程特征映射回原始特征并累加SHAP值
if exist('engineered_feature_names', 'var') && exist('feature_names', 'var')
    % 创建一个映射，将工程特征索引映射到原始特征名称
    n_engineered_features = length(engineered_feature_names);
    n_original_features = length(feature_names);
    
    % 初始化原始特征的SHAP值累加器
    original_shap_values = zeros(1, n_original_features);
    original_feature_contributions = zeros(1, n_original_features);
    
    % 根据工程特征的命名规则将它们映射回原始特征
    for i = 1:n_engineered_features
        eng_feature_name = engineered_feature_names{i};
        
        % 确定该工程特征属于哪个原始特征
        original_idx = 0;
        if startsWith(eng_feature_name, 'season_')
            % 季节特征特殊处理，将其归入一个特殊类别
            continue; % 暂时跳过季节特征
        elseif endsWith(eng_feature_name, '_lag')
            % 滞后特征
            base_name = extractBefore(eng_feature_name, '_lag');
            original_idx = find(contains(feature_names, base_name), 1);
        elseif endsWith(eng_feature_name, '_diff')
            % 差分特征
            base_name = extractBefore(eng_feature_name, '_diff');
            original_idx = find(contains(feature_names, base_name), 1);
        elseif endsWith(eng_feature_name, '_arima_resid')
            % ARIMA残差特征
            base_name = extractBefore(eng_feature_name, '_arima_resid');
            original_idx = find(contains(feature_names, base_name), 1);
        else
            % 原始特征
            original_idx = find(contains(feature_names, eng_feature_name), 1);
        end
        
        % 累加SHAP值到对应的原始特征
        if original_idx > 0 && original_idx <= n_original_features
            original_shap_values(original_idx) = original_shap_values(original_idx) + shap_percentages(i);
        end
    end
    
    % 按SHAP值大小排序原始特征
    [sorted_original_shap, original_shap_idx] = sort(original_shap_values, 'descend');
    
    % 筛选重要性大于0的特征（避免显示没有贡献的特征）
    significant_features = sorted_original_shap > 0;
    sorted_shap = sorted_original_shap(significant_features);
    shap_idx = original_shap_idx(significant_features);
    
    % 获取对应的原始特征名称
    feature_labels = feature_names(shap_idx);
    
    fprintf('已将工程特征映射回原始特征并累加SHAP值\n');
else
    % 如果没有工程特征名称，则使用原来的方法
    % 按SHAP值大小排序
    ...
    % 获取对应的特征名称
    % 修复索引越界问题
    if exist('engineered_feature_names', 'var') && max(shap_idx) <= length(engineered_feature_names)
        feature_labels = engineered_feature_names(shap_idx);
    elseif max(shap_idx) <= length(feature_names)
        feature_labels = feature_names(shap_idx);
    else
        % 如果索引超出了所有特征名称的范围，则生成默认的特征名称
        feature_labels = arrayfun(@(x) sprintf('Feature_%d', x), shap_idx, 'UniformOutput', false);
        warning('SHAP索引超出了特征名称范围，使用默认特征名称');
    end
end
...
for i = 1:length(sorted_shap)
    ...
    text(i, -max(sorted_shap) * 0.01, feature_labels{i}, ...
        'HorizontalAlignment', 'center', 'FontSize', 6, 'Color', 'k', 'VerticalAlignment', 'top');
end
...
%% 10. SHAP依赖图（SHAP Dependence Plot）
% 选择最重要的特征进行依赖图分析
top_feature_idx = shap_idx(1);  % 选择最重要的特征
if exist('engineered_feature_names', 'var') && top_feature_idx <= length(engineered_feature_names)
    feature_name = engineered_feature_names{top_feature_idx};
else
    feature_name = feature_names{top_feature_idx};
end
feature_data = p_test(sample_indices, top_feature_idx);

% 创建SHAP依赖图
figure('Name', strcat('SHAP Dependence Plot - ', feature_name), 'Position', [100, 100, 1200, 800]);

% 计算点的密度以确定透明度
% 将数据网格化
x = feature_data;
y = shap_values(:, 1);
[N, X, Y] = histcounts2(x, y, 50);  % 50x50的网格
...
for i = 1:length(x)
    ...
    elseif y(i) >= Y(end)
        ...
        [~, yIdx(i)] = min(abs(Y(1:end-1) - y(i)));
    end
end

% 根据密度计算透明度（密度越高，透明度越低）
density = N(sub2ind(size(N), yIdx, xIdx));
maxDensity = max(density);
alphas = 0.2 + 0.8 * (1 - density / maxDensity);  % 透明度范围0.2-1.0

% 绘制散点图：根据密度动态调整透明度
scatter(x, y, 30, y, 'filled', 'MarkerEdgeColor', 'none');

% 设置每个点的透明度
h = gca;
h.Children.MarkerFaceAlpha = 'flat';
h.Children.AlphaData = alphas;
...
xlim([min(feature_data) - 0.1*x_range, max(feature_data) + 0.1*x_range]);
ylim([-0.06, 0.06]);

% 设置坐标轴样式 - 适当增加线宽使图表更清晰
set(gca, 'Box', 'on', 'LineWidth', 1.2);  % 线宽从1.5调整为1.2，更协调

% 保存结果
save_variables = {'best_params', 'best_booster', 'best_iteration', ...
     'train_acc', 'test_acc', 'train_precision', 'test_precision', ...
     'train_recall', 'test_recall', 'train_f1', 'test_f1', ...
     'train_auc_final', 'test_auc', 'train_logloss', 'test_logloss', ...
     'ps_input', 'best_threshold', 'overfit_auc', 'overfit_f1', ...
     'ensemble_train_prob', 'ensemble_test_prob', ...
     'predicted_train_classes', 'predicted_test_classes', ...
     'cv_auc', 'cv_f1', 'cv_recall', 'cv_accuracy', 'lstm_model', ...
     'shap_values', 'mean_shap', 'shap_percentages', 'shap_idx', 'sorted_shap'};
